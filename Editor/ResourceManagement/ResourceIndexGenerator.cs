...
{
    private string scanRoot = GameAssetsPathDefine.RuntimeAssetsRoot;
    private string outputSOPath = GameAssetsPathDefine.ResourceIndexSOEditorDefaultPath;
    private string outputJsonPath = GameAssetsPathDefine.ResourceIndexJsonPublishPath;
    private string resKeysPath = GameAssetsPathDefine.ResKeysEditorDefaultPath;
    ...
    {
        // 加载之前保存的路径
        scanRoot = EditorPrefs.GetString("RIG_ScanRoot", GameAssetsPathDefine.RuntimeAssetsRoot);
        outputJsonPath = EditorPrefs.GetString("RIG_JsonPath", GameAssetsPathDefine.ResourceIndexJsonPublishPath);
        resKeysPath = EditorPrefs.GetString("RIG_ResKeysPath", GameAssetsPathDefine.ResKeysEditorDefaultPath);
        outputSOPath = EditorPrefs.GetString("RIG_SOPath", GameAssetsPathDefine.ResourceIndexSOEditorDefaultPath);
    }
    ...
    private void OnGUI()
    {
        ...
        GUILayout.Label("Proto 配置", EditorStyles.boldLabel);
        EditorGUILayout.HelpBox(
            "点击下方按钮将根据 .proto 文件重新生成 all.json 并拷贝到 StreamingAssets。",
            MessageType.Info);

        if (GUILayout.Button("生成 Proto all.json", GUILayout.Height(40)))
        {
            ProtoAllJsonGenerator.Regenerate();
        }
    }
    ...
    static bool IsSupported(string ext)
    {
        return ext == ".prefab" || ext == ".mat" || ext == ".png" || ext == ".asset" ||
                ...
                ext == ".js" || ext == ".unity" || ext == ".scene" || ext == ".bbs" || ext == ".rltif" || ext == ".mp4";
    }
