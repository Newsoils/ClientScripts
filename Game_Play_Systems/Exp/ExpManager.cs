    ...
    namespace Project_Mouse
    {
        namespace Game_Play_System
        {
            ...
            public class ExpManager : MonoBehaviour
            {
                ...
                {
                    instance = this;
                    string text = JsonDataManager.Load_Single_JsonData("project_mouse_tb_friendship_info");
                ...
                //     EvtDsp.TriggerEvt<string, string>(EvtNames.Send_Req_To_Server, "Save_Player_Level", data);
                // }
                private void LoadFromServer()
                {
                    var roleInfo = Global_Game_Manager.Instance.get_current_player_role_info();
                    curLevel = roleInfo.Lv;
                    curExp = (int)roleInfo.Exp;
                    Log.Info("获取等级数据成功");
                    // Log.Info(roleInfo);
                ...
                public void OnGetExpFromServer()
                {
                    var roleInfo = Global_Game_Manager.Instance.get_current_player_role_info();
                    curLevel = roleInfo.Lv;
                    curExp = (int)roleInfo.Exp;
                    Log.Info("等级经验变化,当前等级：" + curLevel + ",当前经验：" + curExp);
                    EvtDsp.TriggerEvt(EvtNames.On_Get_Exp);
                }
